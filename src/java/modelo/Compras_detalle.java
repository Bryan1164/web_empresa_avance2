...
public class Compras_detalle extends Producto {
 ...
 try{
     ...
     //base al encabezado de la tabla del SQL//
      String query = "SELECT p.id_producto as id,p.producto,p.descripcion,p.imagen,p.precio_costo,p.precio_venta,p.existencia,p.fecha_ingreso,c.cantidad,c.id_compra FROM productos as p inner join compras_detalle as c on p.id_producto = c.id_producto;";
      ResultSet consulta = cn.conexionBD.createStatement().executeQuery(query);
      String encabezado[] = {"id","producto","descripcion","imagen","precio_costo","precio_venta","existencia","fecha_ingreso","cantidad","id_compra"};
      ...
      while (consulta.next()){
          ...
          datos[7] = consulta.getString("id_compra");
          tabla.addRow(datos);
         }
        cn.cerrar_conexion();
 ...
 return tabla;
   ...
   try{
       ...
       parametro.setInt(8, getId_compra_detalle());
       parametro.setInt(9, getId_compra());
   ...
   try{
       PreparedStatement parametro;
       cn = new Conexion();
       String query = "UPDATE compras_detalle SET id_compras_detalle=?,id_compra=?,id_producto=?,cantidad=?,costo_precio_unitario=? WHERE id_compras_detalle=?;";
       cn.abrir_conexion();
       parametro = (PreparedStatement)cn.conexionBD.prepareStatement(query);
      parametro.setString(1, getProducto());
       parametro.setString(2, getDescripcion());
